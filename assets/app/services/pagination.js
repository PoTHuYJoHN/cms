(function() {
	...
	paging.$inject = ['HttpService', '$location', '$state', '$stateParams', '$timeout', '$rootScope'];
	function paging(HttpService, $location, $state, $stateParams, $timeout, $rootScope) {
		...
		function internalAction(scope, page) {
			...
			HttpService.getWParams(scope.url, { params: $location.search() } , function(data){
				...
				scope.page = data.meta.pagination.current_page || 1;
				scope.pageSize = data.meta.pagination.per_page || 12;
				scope.total = data.meta.pagination.total || 0;
		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				//scope.$root.$on('filter:change', function(event, newParams, callback){
				//	_.each(newParams, function(val, key){
				//		$stateParams[key] = val;
				//		$location.search(key, val);
				//	});
				//
				//	internalAction(scope, $stateParams.page || 1);
				//
				//	if(callback) {
				//		callback();
				//	}
				...
				$rootScope.filter = $location.search();

				var stopWatch = $rootScope.$watchCollection('filter', function(newValue, oldValue){
					...
					if (newValue !== oldValue) {
						...
						var oldParams = _.clone(oldValue);
				...
				var stopOn =$rootScope.$on('filter:clean', function(){
					_.each($rootScope.filter, function(val, key){
						$rootScope.filter[key] = null;
				...
				//scope.$root.$watch(function(){ return $stateParams; }, function(newValue, oldValue){
				//
				//	var newParams = _.clone(newValue);
				//	var oldParams = _.clone(oldValue);
				//
				...
				//	console.log(1);
				//
				//	if(!_.isEqual(newValue, scope.$root.filter)) {
				//		$state.current.reloadOnSearch = false;
				//		$state.go($state.current, $location.search(), {reload: true});
				//		$state.reload();
				//	}
				//
				//}, true);
